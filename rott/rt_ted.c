...
void PreCachePlayerSound( void ) {
	...
	case 0:SD_PreCacheSound( SD_PLAYERTCSND );

		break;
	case 1:SD_PreCacheSound( SD_PLAYERTBSND );

		break;
	case 2:SD_PreCacheSound( SD_PLAYERDWSND );

		break;
	case 3:SD_PreCacheSound( SD_PLAYERLNSND );

		break;
	case 4:SD_PreCacheSound( SD_PLAYERIPFSND );
		break;
	}
}
...
void PreCacheActor( int actor, int which ) {
	...
	case wallopobj: start = W_GetNumForName( "BSTAR1" );
	...
	case boulderobj: start = W_GetNumForName( "BOL11" );
	...
	case spearobj: SD_PreCacheSound( SD_SPEARSTABSND );
		...
		for ( i = 1; i < cacheindex; i++ ) {
			...
			while ( lastmem <= currentmem ) {//SetTextMode (  );
				if ( iGLOBAL_SCREENWIDTH == 320 ) {
					...
					DrawNormalSprite( 72 + 3 + (Gs * (lastmem << 2)),
									  446 + 3,
									  W_GetNumForName( "led1" ));//led1 progressbar
					...
					DrawNormalSprite( 91 + 3 + (Gs * (lastmem << 2)),
									  559 + 3,
									  W_GetNumForName( "led1" ));//led1 progressbar
				}
			...
			while ( lastcache <= currentcache ) {
				...
				if ( iGLOBAL_SCREENWIDTH == 320 ) {
					...
					DrawNormalSprite( 72 + (Gs * (lastcache << 2)),
									  458 + 3,
									  W_GetNumForName( "led2" ));//led2 progressbar
					DrawNormalSprite( 72 + 3 + (Gs * (lastcache << 2)),
									  458,
									  W_GetNumForName( "led2" ));//led2 progressbar
					DrawNormalSprite( 72 + 3 + (Gs * (lastcache << 2)),
									  458 + 3,
					...
					DrawNormalSprite( 91 + (Gs * (lastcache << 2)),
									  573 + 3,
									  W_GetNumForName( "led2" ));//led2 progressbar
					DrawNormalSprite( 91 + 3 + (Gs * (lastcache << 2)),
									  573,
									  W_GetNumForName( "led2" ));//led2 progressbar
					DrawNormalSprite( 91 + 3 + (Gs * (lastcache << 2)),
									  573 + 3,
...
void SetupPlayers( void ) {
	...
	for ( j = 0; j < mapheight; j++ )
		for ( i = 0; i < mapwidth; i++ ) {
			...
			case 275:
			case 276:
			case 277: SPAWNLOC[NUMSPAWNLOCATIONS].x = i;
				SPAWNLOC[NUMSPAWNLOCATIONS].y = j;
	...
	for ( j = 0; j < mapheight; j++ ) {
		for ( i = 0; i < mapwidth; i++ ) {
			...
			} else if ((tile >= 176) && (tile <= 178)) // Multi shot out glassed walls
			{
				...
				switch ( tile ) {
				case 162:SpawnMaskedWall( i, j, mw_normal1, MW_SHOOTABLE | MW_BLOCKING );
					break;
				case 163:SpawnMaskedWall( i, j, mw_normal1, MW_BLOCKING );
					break;
				case 164:SpawnMaskedWall( i, j, mw_normal2, MW_SHOOTABLE | MW_BLOCKING );
					break;
				case 165:SpawnMaskedWall( i, j, mw_normal2, MW_BLOCKING );
					break;
				case 166:SpawnMaskedWall( i, j, mw_normal3, MW_SHOOTABLE | MW_BLOCKING );
					break;
				case 167:SpawnMaskedWall( i, j, mw_normal3, MW_BLOCKING );
					break;
				case 168:SpawnMaskedWall( i, j, mw_singlepane, MW_SHOOTABLE | MW_BLOCKINGCHANGES | MW_BLOCKING );
					break;
				case 169:SpawnMaskedWall( i, j, mw_singlepane, MW_BOTTOMPASSABLE );
					break;
				case 170:SpawnMaskedWall( i, j, mw_dogwall, MW_NONDOGBLOCKING | MW_WEAPONBLOCKING );
					break;
				case 171:SpawnMaskedWall( i, j, mw_peephole, MW_WEAPONBLOCKING | MW_BLOCKING );
					break;
				}
			} else if ( tile == 172 )
		...
		for ( i = 0; i < mapwidth; i++ ) {
			if ( IsPlatform( i, j )) // tall platform in icon plane
			{
					...
					// check to see we are not on a wall
				{
					...
					switch ( which ) {
					case 0:SpawnMaskedWall( i, j, mw_platform1, MW_BOTTOMPASSABLE | MW_MIDDLEPASSABLE );
						break;
					case 1:SpawnMaskedWall( i, j, mw_platform2, MW_ABOVEPASSABLE | MW_MIDDLEPASSABLE );
						break;
					case 2:SpawnMaskedWall( i, j, mw_platform3, MW_MIDDLEPASSABLE );
						break;
					case 3:SpawnMaskedWall( i, j, mw_platform4, MW_BOTTOMPASSABLE );
						break;
					case 4:SpawnMaskedWall( i, j, mw_platform5, MW_BOTTOMPASSABLE | MW_ABOVEPASSABLE );
						break;
					case 5:SpawnMaskedWall( i, j, mw_platform6, MW_ABOVEPASSABLE );
						break;
					case -3:SpawnMaskedWall( i, j, mw_platform7, MW_ABOVEPASSABLE );
						break;
...
void LinkActor( objtype * ob, int tilex, int tiley,
	...
	if ( !clocklinked ) {
		...
		if ((MISCVARS->TOMLOC.x == touchx) && (MISCVARS->TOMLOC.y == touchy)) {
			...
			Error( "tried to link an object at x=%d y=%d to a non-existent touchplate supposedly at x=%d y=%d",
				   tilex,
				   tiley,
				   touchx,
				   touchy );
...
//               if ( gamestate.Product != ROTT_SHAREWARE )
			...
			case 285:
			case 286:
			case 287:SpawnPushColumn( i, j, tile - 285, nodir, 0 );
				break;
			}
		}
	}
	...
	for ( j = 0; j < mapheight; j++ ) {
		for ( i = 0; i < mapwidth; i++ ) {
			...
			case 303:
			case 304:
			case 305:SpawnPushColumn( i, j, tile - 303, east, linked );
				swapaction = NULL;
				if ( linked )
					LinkActor( new, i, j, action, swapaction );
			...
			case 321:
			case 322:
			case 323:SpawnPushColumn( i, j, tile - 321, north, linked );
				swapaction = NULL;
				if ( linked )
					LinkActor( new, i, j, action, swapaction );
			...
			case 339:
			case 340:
			case 341:SpawnPushColumn( i, j, tile - 339, west, linked );
				swapaction = NULL;
				if ( linked )
					LinkActor( new, i, j, action, swapaction );
			...
			case 357:
			case 358:
			case 359:SpawnPushColumn( i, j, tile - 357, south, linked );
				swapaction = NULL;
				if ( linked )
					LinkActor( new, i, j, action, swapaction );
	...
	if ((!BATTLEMODE) || (gamestate.BattleOptions.SpawnDangers)) {
		...
		for ( j = 0; j < mapheight; j++ ) {
			for ( i = 0; i < mapwidth; i++ ) {
				...
				switch ( tile ) {
				case 89:SpawnFourWayGun( i, j );
					break;

				case 156:
				case 157:SpawnBlade( i, j, nodir, 0, tile - 156 );
					if ( linked )
						LinkActor( new, i, j, action, swapaction );
				...
				case 174:
				case 175:SpawnBlade( i, j, nodir, 1, tile - 174 );
					if ( linked )
						LinkActor( new, i, j, action, swapaction );

					break;

				case 412:SpawnSpear( i, j, 0 );
					break;

				case 430:SpawnSpear( i, j, 1 );
					break;

				case 413:SpawnCrushingColumn( i, j, 1 ); //down
					break;

				case 431:SpawnCrushingColumn( i, j, 0 ); // up
					break;
				...
				case 301:
				case 302:SpawnBlade( i, j, east, tile - 301, 0 );
					if ( !linked )
						new->flags |= FL_ACTIVE;
					else
						LinkActor( new, i, j, action, swapaction );
				...
				case 319:
				case 320:SpawnBlade( i, j, north, tile - 319, 0 );
					if ( !linked )
						new->flags |= FL_ACTIVE;
					else
						LinkActor( new, i, j, action, swapaction );
				...
				case 337:
				case 338:SpawnBlade( i, j, west, tile - 337, 0 );
					if ( !linked )
						new->flags |= FL_ACTIVE;
					else
						LinkActor( new, i, j, action, swapaction );
				...
				case 355:
				case 356:SpawnBlade( i, j, south, tile - 355, 0 );
					if ( !linked )
						new->flags |= FL_ACTIVE;
					else
						LinkActor( new, i, j, action, swapaction );
				...
				case 372:SpawnFirejet( i, j, nodir, 0 );
					break;

				case 373:
				case 374:
				case 375:
				case 376:SpawnFirejet( i, j, tile - 373, 0 );
					break;

				case 390:SpawnFirejet( i, j, nodir, 1 );
					break;

				case 391:
				case 392:
				case 393:
				case 394:SpawnFirejet( i, j, tile - 391, 1 );
					break;

				case 278:
				case 279:
				case 280:
				case 281:SpawnBoulder( i, j, tile - 278 );
					if ( !linked )
						new->flags |= FL_ACTIVE;
					else
						LinkActor( new, i, j, action, swapaction );
...
// Initialize Lights in Area
	...
	for ( j = 0; j < mapheight; j++ ) {
		...
		for ( i = starti; i < mapwidth; i++ ) {
			...
			case 42:
			case 63:
			case 64: sprites[i][j]->flags |= FL_LIGHTON;
				break;
...
char * WeaponName( int tile ) {
	...
	case 48:return "Double Pistol ";
		break;

	case 49:return "MP40          ";
		break;

	case 50:return "Bazooka       ";
		break;

	case 51:return "Firebomb      ";
		break;

	case 52:return "Heatseeker    ";
		break;

	case 53:return "Drunk Missile ";
		break;

	case 54:return "Flamewall     ";
		break;

	case 55:return "Split Missile ";
		break;
...
void DoLowMemoryConversionForegroundPlane( void ) {
	...
	for ( j = 0; j < mapheight; j++ ) {
		for ( i = 0; i < mapwidth; i++ ) {
			...
			case 156:
			case 157:*map = 372;    //spinblade stabbers to firejets
				break;
...
void DoRegisterConversionBackgroundPlane( void ) {
	...
	for ( j = 0; j < mapheight; j++ ) {
		for ( i = 0; i < mapwidth; i++ ) {
			...
			case 232:*map = 231; //map chains to machinery
				break;

			case 228:*map = 230; //map gray water to blue water
				break;
...
void SetupRandomActors( void ) {
	...
	case gd_baby:totalrandom = 7 * totalrandom / 10;
		break;

	case gd_easy:totalrandom = 8 * totalrandom / 10;
		break;

	case gd_medium:totalrandom = 9 * totalrandom / 10;
		break;
...
void SetupActors( void ) {
	...
	for ( j = 0; j < mapheight; j++ ) {
		...
		for ( i = starti; i < mapwidth; i++ ) {
			...
			case 109:
			case 110:
			case 111: SpawnStand( lowguardobj, i, j, tile - 108, 0 );
				break;
			...
			case 113:
			case 114:
			case 115: SpawnPatrol( lowguardobj, i, j, tile - 112 );
				break;
			...
			case 117:
			case 118:
			case 119: SpawnStand( lowguardobj, i, j, tile - 116, 1 );
				break;
			...
			case 145:
			case 146:
			case 147: SpawnStand( highguardobj, i, j, tile - 144, 0 );
				break;
			...
			case 153:
			case 154:
			case 155: SpawnStand( highguardobj, i, j, tile - 152, 1 );
				break;
			...
			case 149:
			case 150:
			case 151: SpawnPatrol( highguardobj, i, j, tile - 148 );
				break;
			...
			case 159:
			case 160:
			case 161: SpawnPatrol( roboguardobj, i, j, tile - 158 );
				break;
			...
			case 195:
			case 196:
			case 197: SpawnGunThingy( patrolgunobj, i, j, tile - 194 );
				break;
			...
			case 181:
			case 182:
			case 183: SpawnStand( strikeguardobj, i, j, tile - 180, 0 );
				break;
			...
			case 189:
			case 190:
			case 191: SpawnStand( strikeguardobj, i, j, tile - 188, 1 );
				break;
			...
			case 185:
			case 186:
			case 187: SpawnPatrol( strikeguardobj, i, j, tile - 184 );
				break;
			...
			case 217:
			case 218:
			case 219: SpawnStand( overpatrolobj, i, j, tile - 216, 0 );
				break;
			...
			case 225:
			case 226:
			case 227: SpawnStand( overpatrolobj, i, j, tile - 224, 1 );
				break;
			...
			case 221:
			case 222:
			case 223: SpawnPatrol( overpatrolobj, i, j, tile - 220 );
				break;
			case 306:
			case 307:
			case 308:
			...
			case 289:
			case 290:
			case 291: SpawnStand( triadenforcerobj, i, j, tile - 288, 0 );
				break;
			...
			case 297:
			case 298:
			case 299: SpawnStand( triadenforcerobj, i, j, tile - 296, 1 );
				break;
			...
			case 293:
			case 294:
			case 295: SpawnPatrol( triadenforcerobj, i, j, tile - 292 );
				break;
			...
			case 325:
			case 326:
			case 327: SpawnStand( blitzguardobj, i, j, tile - 324, 0 );
				break;
			...
			case 333:
			case 334:
			case 335: SpawnStand( blitzguardobj, i, j, tile - 332, 1 );
				break;
			...
			case 329:
			case 330:
			case 331: SpawnPatrol( blitzguardobj, i, j, tile - 328 );
				break;
			...
			case 361:
			case 362:
			case 363: SpawnStand( deathmonkobj, i, j, tile - 360, 0 );
				break;
			...
			case 369:
			case 370:
			case 371: SpawnStand( deathmonkobj, i, j, tile - 368, 1 );
				break;
			...
			case 365:
			case 366:
			case 367: SpawnPatrol( deathmonkobj, i, j, tile - 364 );
				break;
			...
			case 397:
			case 398:
			case 399: SpawnStand( dfiremonkobj, i, j, tile - 396, 0 );
				break;
			...
			case 405:
			case 406:
			case 407: SpawnStand( dfiremonkobj, i, j, tile - 404, 1 );
				break;
			...
			case 401:
			case 402:
			case 403: SpawnPatrol( dfiremonkobj, i, j, tile - 400 );
				break;

			case 99: SpawnStand( b_darianobj, i, j, tile - 99, 0 );
				break;
			case 100: SpawnStand( b_heinrichobj, i, j, tile - 100, 0 );
				break;
			case 101: SpawnStand( b_darkmonkobj, i, j, tile - 101, 0 );
				MISCVARS->TOMLOC.x = i;
				MISCVARS->TOMLOC.y = j;
				break;
			case 102: SpawnMultiSpriteActor( b_robobossobj, i, j, tile - 102 );
				break;

			case 103: SpawnSnake( i, j );
				break;
			...
			case 409:
			case 410:
			case 411: SpawnPatrol( wallopobj, i, j, tile - 408 );
				break;
...
void SetupStatics( void ) {
	...
	for ( j = 0; j < mapheight; j++ ) {
		...
		for ( i = starti; i < mapwidth; i++ ) {
			...
			case 43:
			case 63:
			case 64: SpawnStatic( i, j, tile - 23, spawnz );
				break;

			case 44: SpawnStatic( i, j, tile - 23, spawnz );
				if ( loadedgame == false ) {
			...
			case 37:
			case 38:
			case 39: SpawnStatic( i, j, tile - 23, spawnz );
				if ( loadedgame == false )
					gamestate.healthtotal++;
				break;
			...
			case 40:
			case 41:
			case 45: SpawnStatic( i, j, tile - 23, spawnz );
				break;
			...
			case 48: SD_PreCacheSound( SD_ATKTWOPISTOLSND );
			...
			case 50: SD_PreCacheSound( SD_MISSILEHITSND );
			...
			case 51:
				...
				if ( loadedgame == false )
					gamestate.missiletotal++;
				break;
			case 52: SD_PreCacheSound( SD_MISSILEHITSND );
				SD_PreCacheSound( SD_MISSILEFLYSND );
				...
				if ( loadedgame == false )
					gamestate.missiletotal++;
				break;
			case 53:SD_PreCacheSound( SD_MISSILEHITSND );
				SD_PreCacheSound( SD_MISSILEFLYSND );
				...
				if ( loadedgame == false )
					gamestate.missiletotal++;
				break;
			case 54: SD_PreCacheSound( SD_MISSILEHITSND );
				SD_PreCacheSound( SD_MISSILEFLYSND );
			...
			case 65:
			case 66:
			case 67: SpawnStatic( i, j, tile - 23, spawnz );
				break;

			case 68:
			case 69:
			case 70:
			case 71: SpawnStatic( i, j, tile - 23, spawnz );
				break;

			case 98: SpawnStatic( i, j, tile - 98 + 49, spawnz );
				break;

			case 210: SpawnStatic( i, j, stat_scotthead, spawnz );
				break;
			...
			case 231:
			case 232:
			case 233: SpawnStatic( i, j, tile - 228 + 51, spawnz );
				break;
			case 246:
			case 247:
			case 248:
			case 249:
			case 250:
			case 251: SpawnStatic( i, j, tile - 246 + 57, spawnz );
				break;
			case 264:
			case 265: SpawnStatic( i, j, tile - 264 + 63, spawnz );
				gamestate.planttotal++;
				break;

			case 266: SpawnStatic( i, j, stat_urn, spawnz );
				break;

			case 268:SpawnStatic( i, j, tile - 265 + 63, spawnz );
				break;

			case 269: SpawnStatic( i, j, tile - 265 + 63, spawnz );
				break;

			case 267: SpawnStatic( i, j, stat_emptystatue, spawnz );
				break;

			case 282: SpawnStatic( i, j, stat_heatgrate, spawnz );
				break;

			case 283: SpawnStatic( i, j, stat_standardpole, spawnz );
				break;
			...
			case 252: SD_PreCacheSound( SD_GRAVSND );
				SD_PreCacheSound( SD_GRAVHITSND );
			...
			case 254: SpawnStatic( i, j, stat_fleetfeet, spawnz );
				if ( loadedgame == false )
					gamestate.supertotal++;
				break;

			case 255: SpawnStatic( i, j, stat_random, spawnz );
				if ( loadedgame == false )
					gamestate.supertotal++;
				break;

			case 260: SpawnStatic( i, j, stat_elastic, spawnz );
				if ( loadedgame == false )
					gamestate.supertotal++;
				break;

			case 261: SpawnStatic( i, j, stat_mushroom, spawnz );
				if ( loadedgame == false ) {
					gamestate.supertotal++;
					gamestate.democratictotal++;
				}
			...
			case 262: SpawnStatic( i, j, stat_tomlarva, spawnz );
				break;
			...
			case 270: SpawnStatic( i, j, stat_bulletproof, spawnz );
				if ( loadedgame == false )
					gamestate.supertotal++;
				break;
			case 271: SpawnStatic( i, j, stat_asbesto, spawnz );
				if ( loadedgame == false )
					gamestate.supertotal++;
				break;
			case 272: SpawnStatic( i, j, stat_gasmask, spawnz );
				if ( loadedgame == false )
					gamestate.supertotal++;
				break;
			case 461: SpawnStatic( i, j, stat_disk, spawnz );
				break;
			}
		}
	}
