...
int PlatformHeight( int tilex, int tiley ) {
	...
	switch ( platform ) {
	case 1:return -10;
	case 4:return nominalheight;
	case 5:
	case 6:return nominalheight - 64;
	case 7:return nominalheight;
	case 8:
	case 9:return -10;
	}
...
void Link_To_Touchplate( word touchlocx,
						 word touchlocy,
						 void (* maction)( long ),
...
void SpawnDoor( int tilex, int tiley, int lock, int texture ) {
	...
	case 0:
	case 8:basetexture = W_GetNumForName( "RAMDOOR1\0" );
		break;

	case 1:
	case 9:basetexture = W_GetNumForName( "DOOR2\0" );
		break;

	case 2:
	case 3:
	case 13:basetexture = W_GetNumForName( "TRIDOOR1\0" );
		break;

	case 10:
	case 11:
	case 14:basetexture = W_GetNumForName( "SDOOR4\0" );
		break;

	case 12:basetexture = W_GetNumForName( "EDOOR\0" );
		break;
	case 15:basetexture = W_GetNumForName( "SNDOOR\0" );
		break;
	case 16:basetexture = W_GetNumForName( "SNADOOR\0" );
		break;
	case 17:basetexture = W_GetNumForName( "SNKDOOR\0" );
		break;

	case 18:basetexture = W_GetNumForName( "TNDOOR\0" );
		break;
	case 19:basetexture = W_GetNumForName( "TNADOOR\0" );
		break;
	case 20:basetexture = W_GetNumForName( "TNKDOOR\0" );
		break;
...
//
	...
	case 12:
	case 13:
	case 14:lastdoorobj->sidepic = W_GetNumForName( "SIDE8" );
...
void UseDoor( int door ) {
	...
	case dr_closing: SD_StopSound( doorobjlist[door]->soundhandle );
	case dr_closed:UtilizeDoor( door, OpenDoor );
		break;
	case dr_opening: SD_StopSound( doorobjlist[door]->soundhandle );
	case dr_open:
		if ( DoorReadyToClose( door ) == true )
			UtilizeDoor( door, CloseDoor );
		break;
...
void SpawnMaskedWall( int tilex, int tiley, int which, int flags ) {
  ...
  */
	...
	case mw_normal1:side = W_GetNumForName( "SIDE21" );
	...
	case mw_normal2:side = W_GetNumForName( "SIDE21" );
	...
	case mw_normal3:side = W_GetNumForName( "SIDE21" );
	...
	case mw_railing:sidepic = false;
		middle = -1;
		above = -1;
		bottom = W_GetNumForName( "RAILING" );
		break;

	case mw_hiswitchon:sidepic = false;
		middle = himask + 1;
		above = himask + 3;
		bottom = himask;
		break;

	case mw_hiswitchoff:sidepic = false;
		middle = himask + 1;
		above = himask + 2;
		bottom = himask;
		break;

	case mw_entrygate:side = W_GetNumForName( "SIDE21" );
	...
	case mw_platform1:sidepic = false;
		bottom = -1;
		middle = -1;
		above = himask + 10;
		if ( metal == true ) {
		...
		break;
	case mw_platform2:sidepic = false;
		bottom = himask + 8;
		middle = -1;
		above = -1;
		if ( metal == true ) {
		...
		break;
	case mw_platform3:sidepic = false;
		bottom = himask + 8;
		middle = -1;
		above = himask + 10;
		if ( metal == true ) {
			...
			lastmaskobj->flags |= MW_BOTTOMFLIPPING;
		break;
	case mw_platform4:sidepic = false;
		bottom = himask + 12;
		middle = himask + 7;
		above = himask + 7;
		if ( metal == true ) {
		...
		break;
	case mw_platform5:sidepic = false;
		bottom = himask + 12;
		middle = himask + 7;
		above = himask + 5;
		if ( metal == true ) {
		...
		break;
	case mw_platform6:sidepic = false;
		bottom = himask + 4;
		middle = himask + 7;
		above = himask + 5;
		if ( metal == true ) {
			...
			lastmaskobj->flags |= MW_TOPFLIPPING;
		break;
	case mw_platform7:sidepic = false;
		bottom = himask + 4;
		middle = himask + 7;
		above = himask + 5;
		if ((up == 1) || (dn == 1))
...
void CheckElevatorStart( elevator_t * eptr ) {
	...
	if ( dptr->action == dr_closed ) {
		...
		if ( eptr->nextaction != -1 ) {
			...
			eptr->nextaction = -1;
			switch ( eptr->state ) {
			case ev_mtd:eptr->doortoopen = eptr->door2;
			...
			case ev_mts:eptr->doortoopen = eptr->door1;
	...
	for ( ectr = 0; ectr < _numelevators; ectr++ ) {
		...
		if ( eptr->ticcount )
            ...
            case ev_rad:
               break;
            */
			case ev_mts:ExecuteElevatorStopActions( eptr, 0, (eptr->sx << 16), (eptr->sy << 16));
				break;

			case ev_mtd:ExecuteElevatorStopActions( eptr, 1, (eptr->dx << 16), (eptr->dy << 16));
				break;

			case ev_doorclosing:CheckElevatorStart( eptr );
				break;
			}
		}
	}
...
void MoveDoors( void ) {
	...
	for ( door = 0; door < doornum; door++ )
		switch ( doorobjlist[door]->action ) {
		case dr_open: DoorOpen( door );
			break;

		case dr_opening: DoorOpening( door );
			SD_PanRTP( doorobjlist[door]->soundhandle, doorobjlist[door]->tilex << 16, doorobjlist[door]->tiley << 16 );
			break;

		case dr_closing: DoorClosing( door );
			SD_PanRTP( doorobjlist[door]->soundhandle, doorobjlist[door]->tilex << 16, doorobjlist[door]->tiley << 16 );
			break;
		default:;
		}
...
void SpawnPushWall( int tilex, int tiley, int lock, int texture, int dir, int type ) {
	...
	case 0:
	case 1:
	case 3: lastpwallobj->speed = 2;
		break;
	case 2:
	case 4: lastpwallobj->speed = 4;
		break;
	default:Error( "Illegal PushWall type passed into SpawnPushWall\n" );
		break;
	}
	if ( type > 2 ) {
		tilemap[tilex][tiley] = 0;
...
void ActivatePushWall( long pwall ) {
	...
	switch ( pw->action ) {
	case pw_npushed:pw->action = pw_pushing;
		ConnectPushWall( pwall );
		SetupPushWall( pwall );
		gamestate.secretcount++;
		break;
...
void ActivateMoveWall( long pwall ) {
	...
	switch ( pw->action ) {
	case pw_npushed:pw->action = pw_moving;
		SetupPushWall( pwall );
		break;
	default:SD_Play( SD_BADTOUCHSND );
		break;
	}
}
...
void SetupPushWall( int pwall ) {
	...
	switch ( pw->dir ) {
	case north: pw->momentumx = 0;
		pw->momentumy = -speed;
		break;

	case east: pw->momentumx = speed;
		pw->momentumy = 0;
		break;

	case northeast: pw->momentumx = speed;
		pw->momentumy = -speed;
		break;

	case southeast: pw->momentumx = speed;
		pw->momentumy = speed;
		break;

	case south: pw->momentumx = 0;
		pw->momentumy = speed;
		break;

	case west: pw->momentumx = -speed;
		pw->momentumy = 0;
		break;

	case northwest: pw->momentumx = -speed;
		pw->momentumy = -speed;
		break;

	case southwest: pw->momentumx = -speed;
		pw->momentumy = speed;
		break;
	}
