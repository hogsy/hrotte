...
static void _MIDI_MetaEvent
	...
	switch ( command ) {
	case MIDI_END_OF_TRACK :Track->active = FALSE;

		_MIDI_ActiveTracks--;
		break;

	case MIDI_TEMPO_CHANGE :tempo = 60000000L / _MIDI_ReadNumber( Track->pos, 3 );
		MIDI_SetTempo( tempo );
		break;
...
static int _MIDI_InterpretControllerInfo
	...
	switch ( c1 ) {
	case MIDI_MONO_MODE_ON :Track->pos++;
		break;
	...
	case EMIDI_EXCLUDE_TRACK :break;
	...
	while ( tracknum < _MIDI_NumTracks ) {
		while ((Track->active) && (Track->delay == 0)) {
			...
			if ( GET_MIDI_COMMAND( event ) == MIDI_SPECIAL ) {
				switch ( event ) {
				case MIDI_SYSEX :
				case MIDI_SYSEX_CONTINUE :_MIDI_SysEx( Track );
					break;

				case MIDI_META_EVENT :_MIDI_MetaEvent( Track );
					break;
				}
...
static void _MIDI_InitEMIDI
	...
	case SoundBlaster :type = EMIDI_SoundBlaster;
		break;

	case ProAudioSpectrum :type = EMIDI_ProAudio;
		break;

	case SoundMan16 :type = EMIDI_SoundMan16;
		break;

	case Adlib :type = EMIDI_Adlib;
		break;

	case GenMidi :type = EMIDI_GeneralMIDI;
		break;

	case SoundCanvas :type = EMIDI_SoundCanvas;
		break;

	case Awe32 :type = EMIDI_AWE32;
		break;

	case WaveBlaster :type = EMIDI_WaveBlaster;
		break;

	case SoundScape :type = EMIDI_Soundscape;
		break;

	case UltraSound :type = EMIDI_Ultrasound;
		break;
	}
	...
	while ((tracknum < _MIDI_NumTracks) && (Track != NULL)) {
		...
		while ( !Finished ) {
			...
			if ( GET_MIDI_COMMAND( event ) == MIDI_SPECIAL ) {
				switch ( event ) {
				case MIDI_SYSEX :
				case MIDI_SYSEX_CONTINUE :length = _MIDI_ReadDelta( Track );
					Track->pos += length;
					break;

				case MIDI_META_EVENT :GET_NEXT_EVENT( Track, command );
					GET_NEXT_EVENT( Track, length );
