...
void CheckCommandLineParameters( void ) {
	...
	for ( i = 1; i < _argc; i++ ) {
		...
		case 4:warp = true;
			...
			break;
		case 5:dopefish = true;
			break;
		case 6:SCREENSHOTS = true;
			break;
		case 7:MONOPRESENT = true;
			break;
		case 8:MAPSTATS = true;
			break;
		case 9:TILESTATS = true;
			break;
		case 10:SetTextMode();
			printf( "Rise of the Triad  (c) 1995 Apogee Software\n" );
//MED
...
#endif
			break;
		case 14:startlevel = (ParseNum( _argv[i + 1] ) - 1);
			break;
		case 15:IS8250 = true;
			break;
		case 16:vrenabled = true;
			if ( !quiet )
				printf( "Virtual Reality Mode enabled\n" );
			break;
		case 17:timelimitenabled = true;
		...
		case 18:maxtimelimit = ParseNum( _argv[i + 1] );
			maxtimelimit *= VBLCOUNTER;
			break;
		case 19:noecho = true;
			break;
		case 20:demoexit = true;
			break;
		case 21:quiet = true;
			break;
		}
...
void SetupWads( void ) {
	...
	for ( i = 1; i < _argc; i++ ) {
		...
		switch ( arg ) {
		case 0:iG_aimCross = 1;
			break;
		case 1:sdl_fullscreen = 1;
			break;
		case 2:sdl_fullscreen = 0;
			break;
		case 3:i++;
			if ( i < _argc ) {
				int width, height;
...
void GameLoop( void ) {
	...
	while ( 1 ) {
		...
		case ex_stillplaying:InitializeMessages();
		...
		case ex_died:loadit = done = false;
//		   SetTextMode (  ); //12345678
			Died();
			StopWind();
			DisableScreenStretch();//bna++ shut off streech mode
		...
		case ex_bossdied:
			...
			if ((playstate == ex_bossdied) && (gamestate.mapon != 30)) {
				...
				case 6:shape = W_GetNumForName( "deadstev" );
					break;
				case 14:shape = W_GetNumForName( "deadjoe" );
					break;
				case 22:shape = W_GetNumForName( "deadrobo" );
					break;
				case 33:shape = W_GetNumForName( "deadtom" );
					break;
//                  default:
...
//                     break;
				...
				switch ( gamestate.mapon ) {
				case 6:strcpy( &str[0], "\"General\" John Darian" );
					break;
				case 14:strcpy( &str[0], "Sebastian \"Doyle\" Krist" );
					break;
				case 22:strcpy( &str[0], "the NME" );
					break;
				case 33:strcpy( &str[0], "El Oscuro" );
					break;
//                  default:
//                     Error("Boss died on an illegal level\n");
//                     break;
		...
		case ex_demorecord:ShutdownClientControls();
			Z_FreeTags( PU_LEVELSTRUCT, PU_LEVELEND );       // Free current level
		...
		case ex_demoplayback:ShutdownClientControls();
			Z_FreeTags( PU_LEVELSTRUCT, PU_LEVELEND );       // Free current level
...
void UpdateGameObjects( void ) {
	...
	while ( oldpolltime < oldtime ) {
		...
		if ( BattleStatus != battle_no_event ) {
			...
			case battle_out_of_time :playstate = ex_battledone;
				break;

			case battle_end_round :SetWhoHaveWeapons();
				break;
			default:;
			}
			if ( playstate == ex_battledone ) {
...
void PollKeyboard
	...
	if ((MSG.messageon == false) && (!quitactive)) {
		...
		if ((Keyboard[sc_F5]) && ((!BATTLEMODE) ||
			...
			switch ( DetailLevel ) {
			case 0 :AddMessage( "Low detail", MSG_SYSTEM);
				break;

			case 1 :AddMessage( "Medium detail", MSG_SYSTEM);
				break;

			case 2 :AddMessage( "High detail", MSG_SYSTEM);
				break;
			}
			doublestep = 2 - DetailLevel;
		}
