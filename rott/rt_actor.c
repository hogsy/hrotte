...
void RemoveObj( objtype * gone ) {
	...
	if ( gone->obclass != inertobj ) {
		if ( ValidAreanumber( gone->areanumber ))
			...
			Error( "tried to remove an instance of %s with invalid areanumber %d",
				   debugstr[gone->obclass],
				   gone->areanumber );
	}
...
// bosses are "special" ==========================
		...
		if ( victim->hitpoints <= 0 ) {
			...
			case b_darianobj:AddMessage( "Darian defeated!", MSG_CHEAT);
				break;

			case b_heinrichobj:AddMessage( "Krist defeated!", MSG_CHEAT);
				break;

			case b_robobossobj:AddMessage( "NME defeated!", MSG_CHEAT);
				break;
			default:;
			}
				...
				if ( M_ISACTOR( owner )) {
					if ( owner->obclass == playerobj ) {
						if ( !victim->momentumz )
							...
							BATTLE_PlayerKilledPlayer( battle_kill_with_missile_in_air,
													   owner->dirchoosetime,
													   victim->dirchoosetime );
					} else
...
void MissileHit( objtype * ob, void * hitwhat ) {
   ...
   */
	...
	case p_bazookaobj:NewState( ob, &s_explosion1 );
	...
	case p_heatseekobj:NewState( ob, &s_explosion1 );
		damage = 2 * ((random >> 3) + 50);
		break;

	case p_drunkmissileobj:NewState( ob, &s_explosion1 );
		damage = ((random >> 3) + 25);
		break;

	case p_firebombobj:NewState( ob, &s_explosion1 );
	...
	case shurikenobj:NewState( ob, &s_explosion1 );
		damage = ((random >> 3) + 30) >> 2;
		break;

	case grenadeobj:NewState( ob, &s_explosion1 );
		damage = (random >> 3) + 20;
		break;

	case fireballobj:NewState( ob, &s_explosion1 );
		damage = (random >> 3) + 10;
		fireweapon = 1;
		break;

	case missileobj:NewState( ob, &s_explosion1 );
	...
	case inertobj:ob->state = NULL;
		return;
		break;

#if (SHAREWARE == 0)

	case p_splitmissileobj:NewState( ob, &s_explosion1 );
		damage = 2 * ((random >> 3) + 50);
		break;
	...
	case bigshurikenobj:NewState( ob, &s_oshurikenhit1 );
	...
	case dm_spitobj:NewState( ob, &s_spithit1 );
		damage = 30;
		if ( gamestate.difficulty == gd_hard )
			damage += 15;
		break;

	case dm_weaponobj:damage = 20;
		NewState( ob, &s_explosion1 );
		break;

	case dm_heatseekobj:damage = 20;
		NewState( ob, &s_explosion1 );
		break;

	case dmfballobj:NewState( ob, &s_explosion1 );
		damage = (random >> 3) + 20;
		fireweapon = 1;
		break;

	case h_mineobj:NewState( ob, &s_explosion1 );
		damage = (random >> 3) + 20;
		break;

	case NMEsaucerobj:NewState( ob, &s_explosion1 );
...
void MoveActor( objtype * ob ) {
	...
	if ((ob->obclass == playerobj) || (ob->flags & FL_NOFRICTION) || (ob->state->think == T_Collide) ||
		(ob->obclass == b_heinrichobj) || (ob->obclass == h_mineobj)) SetVisiblePosition( ob, ob->x, ob->y );
...
//==========================================================================
		...
		for ( i = 0; i < doornum; i++ ) {
			...
			if ( dptr->vertical ) {
				...
				if ( area1 == ob->areanumber ) SetCollectorTarget( -1, 0, east ) else if ( area2
					== ob->areanumber ) SetCollectorTarget( 1, 0, west );

			} else {
				int area1 = AREANUMBER( dptr->tilex, dptr->tiley - 1 ),
					area2 = AREANUMBER( dptr->tilex, dptr->tiley + 1 );

				if ( area1 == ob->areanumber ) SetCollectorTarget( 0, -1, south ) else if ( area2
					== ob->areanumber ) SetCollectorTarget( 0, 1, north );
			...
			if ( mw->flags & MW_NONDOGBLOCKING ) {
				...
				if ( mw->flags & MW_ABOVEPASSABLE ) {
					...
					} else  //==> middle only
					{
						if ( ob->z > LOWFALLCLIPZ + MAXSTEPHEIGHT )
							MWALLSTOP = true;
						else if ( tryz >= LOWFALLCLIPZ) ClipHeight( ob, LOWFALLCLIPZ ) else {
							if ( ob->z >= HIGHRISECLIPZ ) {
								if ( tryz <= HIGHRISECLIPZ )
									ob->momentumz = 0;
							} else if ( tryz <= HIGHRISECLIPZ )
...
boolean ActorTryMove( objtype * ob, int tryx, int tryy, int tryz ) {
	...
	movement_status( *reduced_movement_check[3] )( objtype * ,
	int,int,int)=
	{
		CheckRegularWalls,
			CheckMaskedWalls,
	...
	movement_status( *complete_movement_check[5] )( objtype * ,
	int,int,int)=
	{
		CheckOtherActors,
			CheckRegularWalls,
	...
	case inertobj:
	case bladeobj:
	case firejetobj:movement_function = &reduced_movement_check[0];
		numcheckfunctions = 3;
		break;

	default:movement_function = &complete_movement_check[0];
		numcheckfunctions = 5;
		break;
	}
...
void T_EsauChase( objtype * ob ) {
	...
	if ((ob->tilex == ob->targettilex) && (ob->tiley == ob->targettiley)) {
		if ( MISCVARS->DSTATE == ESAU_USING_HOLES ) {
			...
			{
				switch ( ob->temp1 ) {
				case east:ob->targettilex++;
					break;
				case west:ob->targettilex--;
					break;
				case north:ob->targettiley--;
					break;
				case south:ob->targettiley++;
					break;
				}
			}
			SelectTouchDir( ob );
...
void SelectTouchDir( objtype * ob ) {
	...
	if ( !MISCVARS->notouch ) {
		...
		switch ( hole ) {
		case down_in_a_hole:return;

		case no_holes_available:MISCVARS->noholes = 1;
			break;

		case holes_unreachable:FindDoor( ob );
			break;
...
void T_NME_HeadShoot( objtype * ob ) {//int randtheta,i,offx,offy;
	...
	if ( tempstat->hitpoints <= 0 ) {
		...
		if ( tempstat->flags & FL_LIGHT ) {
				...
				//SpawnFallingDebris(tempstat->x,tempstat->y,tempstat->z-32);
			{
				...
				case stat_torch:SpawnSlowParticles( gt_sparks, 5, tempstat->x, tempstat->y, tempstat->z - 32 );
					break;
				case stat_floorfire:SpawnSlowParticles( gt_sparks, 5, tempstat->x, tempstat->y, tempstat->z );
					break;
				default:;
				}
			}
			...
			switch ( tempstat->itemnumber ) {
			case stat_dariantouch:MISCVARS->ETOUCH[tempstat->linked_to].x = MISCVARS->ETOUCH[tempstat->linked_to].y = 0;
			case stat_tntcrate:
			case stat_bonusbarrel:SpawnNewObj( tempstat->tilex, tempstat->tiley, &s_staticexplosion1, inertobj );
				MakeActive( new );
				new->flags |= FL_ABP;
				...
				break;
#if (SHAREWARE == 0)
			case stat_mine:SpawnNewObj( tempstat->tilex, tempstat->tiley, &s_grexplosion1, inertobj );
				MakeActive( new );
				new->flags |= FL_ABP;
				new->whatever = tempstat;
				new->temp2 = damage;
			...
			case stat_tomlarva:SD_PlaySoundRTP( SD_ACTORSQUISHSND, tempstat->x, tempstat->y );
			...
			case stat_lifeitem2:
			case stat_lifeitem3:
			case stat_lifeitem4:SD_PlaySoundRTP( SD_ITEMBLOWSND, tempstat->x, tempstat->y );
...
int EnvironmentDamage( objtype * ob ) {
	if ( BATTLEMODE && (gamestate.BattleOptions.DangerDamage != bo_danger_normal)) {
		...
		switch ( ob->obclass ) {
		case firejetobj:
		case bladeobj:damage = 6;
			break;

		case boulderobj:damage = 50;
			break;

		case spearobj:damage = 7;
			break;

		case gasgrateobj:damage = 20;
			break;

		case wallfireobj:damage = ((GameRandomNumber( "wallfire damage", 0 ) >> 3) + 10);
			break;

		case crushcolobj:damage = 10;
			break;
		default:;
		}
...
void A_MissileWeapon( objtype * ob ) {
	...
	if ( !ob->ticcount ) {
		...
		case triadenforcerobj: nobclass = grenadeobj;
			nstate = &s_grenade1;
			sound++;
			break;

		case roboguardobj: nobclass = shurikenobj;
			nspeed = 0x2000;
			noffset = 0x10000;
	  ...
	  */
	  ...
	  case b_robobossobj:
		 ...
		 */
#if (SHAREWARE == 0)
		case b_darianobj: nobclass = missileobj;
			//nspeed = 0x100;
			//noffset = 0x18000;
			nstate = &s_missile1;
			zoffset = -20;
		...
		case dfiremonkobj: nobclass = fireballobj;
			nstate = &s_monkfire1;
			break;

		case overpatrolobj: nobclass = netobj;
			nstate = &s_bolocast1;
			sound++;
			break;
...
void T_BossDied( objtype * ob ) {
	...
	case b_darkmonkobj:
	case b_robobossobj:
	case b_darksnakeobj: playstate = ex_bossdied;
		break;
	default:;
	}
}
