...
void DrawPlayerWeapon( void ) {
	...
	if ( locplayerstate->weapon != -1 ) {
		...
		if ( !(locplayerstate->NETCAPTURED) ||
			...
			case wp_godhand: break;

			case wp_mp40: break;

			case wp_firewall: ydisp = 10;
				break;

			case wp_bazooka:break;

			case wp_heatseeker: ydisp = 20;
				break;

			case wp_pistol: break;

			case wp_twopistol: xdisp = 80;
				break;

			case wp_drunk: ydisp = 10;
				break;

			case wp_firebomb: break;

#if (SHAREWARE == 0)

			case wp_kes: break;

			case wp_bat:xdisp = 20;
				break;

			case wp_split: ydisp = 20;
				break;
			...
			default:Error( "Illegal weapon value = %d\n", locplayerstate->weapon );
				break;
			}
		} else
			xdisp = 60;
...
void SetWallLightLevel( wallcast_t * post ) {
	...
	case 1:la = 4;
		break;
	case 2:la = (4 - gamestate.difficulty);
		break;
	case 3:la = 3 + (4 - gamestate.difficulty);
		break;
	}
...
void DopefishTitle( void ) {
	...
	for ( height = 0; height < FINEANGLES << 1; height += (tics << 5)) {
		DrawPositionedScaledSprite( 160 + FixedMul( 60, costable[height & (FINEANGLES - 1)] ),
									100 + FixedMul( 60, sintable[height & (FINEANGLES - 1)] ),
									shapenum,
									200,
									0 );
		FlipPage();
...
void UpdateScreenSaver( void ) {
	...
	DrawRotatedScreen( ScreenSaver->x,
					   ScreenSaver->y,
					   ( byte * ) bufferofs,
					   ScreenSaver->angle,
					   ScreenSaver->scale,
