...
void BATTLE_Init
	...
	switch ( BattleMode ) {
	case battle_StandAloneGame : KillsEndGame = false;
	...
	case battle_Scavenger : KillsEndGame = false;
		KeepTrackOfKills = false;
		gamestate.BattleOptions.SpawnWeapons = true;
		gamestate.BattleOptions.SpawnHealth = true;
		gamestate.SpawnCollectItems = true;
	...
	case battle_Hunter :PointGoal *= BATTLE_NumberOfTeams;
		KillsEndGame = false;
		KeepTrackOfKills = true;
		BATTLE_It = 0;
	...
	case battle_Eluder : KeepTrackOfKills = false;
		KillsEndGame = false;
	...
	case battle_Deluder : KeepTrackOfKills = false;
		KillsEndGame = false;
		gamestate.SpawnDeluder = true;
		break;

	case battle_CaptureTheTriad :KillsEndGame = false;
		KeepTrackOfKills = false;
		break;
	}
...
battle_status BATTLE_PlayerKilledPlayer
	...
	if ( BattleMode == battle_ScoreMore ) {
		points = 0;
		switch ( reason ) {
		case battle_kill_with_missile :points = 1;
			break;

		case battle_kill_with_bullet :points = 2;
			break;

		case battle_kill_with_missile_in_air :points = 2;
			break;

		case battle_kill_with_bullet_in_air :points = 3;
			break;

		case battle_kill_by_crushing :points = 4;
			break;
		...
		case battle_kill_with_missile_in_air :
		case battle_kill_with_bullet_in_air :
		case battle_kill_by_crushing :WhoKilledWho[killer][victim]++;
			if ( KeepTrackOfKills ) {
				if ( killerteam == victimteam ) {
